...
confFile='/etc/nginx/nginx-secure.conf'

echo "---------"
echo $DB_DOMAIN_NAME

# Check if DB domain exists
if [ -n "$DB_DOMAIN_NAME" ]; then
    echo "With DB Domain"
    confFile='/etc/nginx/nginx-secure-multi.conf'
    echo replacing ___db.my.example.com___/$DB_DOMAIN_NAME
    echo replacing ___DB_APPLICATION_IP___/$PHPMYADMIN_PORT_80_TCP_ADDR
    echo replacing ___DB_APPLICATION_PORT___/$PHPMYADMIN_PORT_80_TCP_PORT
    
    # Put your db domain name into the nginx reverse proxy config.
    sed -i "s/___db.my.example.com___/$DB_DOMAIN_NAME/g" $confFile

    # Add your app's container IP and port into config
    sed -i "s/___DB_APPLICATION_IP___/$PHPMYADMIN_PORT_80_TCP_ADDR/g" $confFile
    sed -i "s/___DB_APPLICATION_PORT___/$PHPMYADMIN_PORT_80_TCP_PORT/g" $confFile
fi  



# Put your domain name into the nginx reverse proxy config.
sed -i "s/___my.example.com___/$MY_DOMAIN_NAME/g" $confFile

# Add LE container IP and port into config
sed -i "s/___LETSENCRYPT_IP___/$LETSENCRYPT_PORT_80_TCP_ADDR/g" $confFile
sed -i "s/___LETSENCRYPT_PORT___/$LETSENCRYPT_PORT_80_TCP_PORT/g" $confFile
sed -i "s/___LETSENCRYPT_HTTPS_IP___/$LETSENCRYPT_PORT_443_TCP_ADDR/g" $confFile
sed -i "s/___LETSENCRYPT_HTTPS_PORT___/$LETSENCRYPT_PORT_443_TCP_PORT/g" $confFile

# Add your app's container IP and port into config
sed -i "s/___APPLICATION_IP___/$APP_PORT_80_TCP_ADDR/g" $confFile
sed -i "s/___APPLICATION_PORT___/$APP_PORT_80_TCP_PORT/g" $confFile

#go!
nginx -s stop
cp $confFile /etc/nginx/nginx.conf
